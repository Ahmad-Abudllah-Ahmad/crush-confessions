...
export default function ConfessionDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchConfession = async () => {
    ...
    try {
      const response = await fetch(
        `/api/confessions?confessionId=${confessionId}`
      );
      ...
      setError(
        err.message || "An error occurred while fetching the confession"
      );
      console.error(err);
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      if (data.mutualReveal) {
        if (data.conversationAlreadyExists) {
          if (
            confirm(
              "Both users have revealed interest! A chat already exists. Would you like to go to your messages?"
            )
          ) {
          ...
          if (
            confirm(
              "Both users have revealed interest! A new chat has been created. Would you like to go to your messages?"
            )
          ) {
            router.push(`/chat?conversationId=${data.conversationId}`);
          }
  ...
  const showRevealButton = () => {
    ...
    if (confession.status === "CONNECTED" || confession.mutualReveal)
      return false;
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        <div className="flex-1 flex items-center justify-center">
          <Loader text="Loading confession..." />
        </div>
      </div>
    );
  }
            ...
            <div className="p-6">
              ...
              <div className="flex flex-wrap justify-between items-center border-t border-gray-200 pt-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    From:{" "}
                    <span className="font-medium">{confession.senderName}</span>
                  </p>
                </div>
