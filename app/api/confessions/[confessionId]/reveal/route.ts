...
export async function POST(
  ...
  try {
    ...
    if (!isTargetUser && !isSender) {
      return NextResponse.json(
        { message: 'You are not authorized to reveal interest in this confession' },
        { status: 403 }
      )
    }
    ...
    if (updatedConfession.senderRevealed && updatedConfession.receiverRevealed) {
      ...
      if (confession.targetUserId) {
        ...
        return NextResponse.json({
          message: 'You have revealed interest, but there is no specific target user.',
          status: 'REVEALED',
          mutualReveal: true,
        });
      }
