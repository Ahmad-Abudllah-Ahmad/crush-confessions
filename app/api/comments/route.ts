...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedComments = comments.map((comment: any) => {
      ...
      const userLiked = comment.commentLikes.some((like: any) => like.userId === currentUser.id);
      
      // Format the replies
      const formattedReplies = comment.replies.map((reply: any) => {
        const replyUserLiked = reply.commentLikes.some((like: any) => like.userId === currentUser.id);
