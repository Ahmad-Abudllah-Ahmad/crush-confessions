...
export async function GET(request: NextRequest) {
  try {
    ...
    const unreadCountsPromises = conversations.map(async (conversation: any) => {
    ...
    const unreadCountsMap = unreadCounts.reduce((acc: Record<string, number>, item: { conversationId: string, count: number }) => {
      ...
      return {
        ...
        lastMessage: conversation.messages[0] 
          ? {
              ...
              isCurrentUser: conversation.messages[0].senderId === currentUser.id,
            } 
          : null,
