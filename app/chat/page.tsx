...
export default function ChatPage() {
  ...
  // Check for conversationId in URL query
  useEffect(() => {
    if (status === 'authenticated' && window.location.search) {
      ...
      if (conversationId) {
        ...
        // Remove the query parameter from URL to avoid issues on refresh
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        <div className="flex-1 flex items-center justify-center">
          <div className="text-xl text-purple-600">Loading conversations...</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold text-purple-800 mb-6">Your Messages</h1>
        ...
        {conversations.length === 0 && !isLoading && !error ? (
          <div className="text-center py-12 bg-white rounded-lg shadow">
            <p className="text-gray-600 mb-4">
              No messages yet. When you and your crush both reveal interest in each other, a chat will appear here.
            </p>
          </div>
        ) : (
                  ...
                  >
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-purple-200 rounded-full flex items-center justify-center text-purple-700 font-semibold">
                        {conversation.otherUser && conversation.otherUser.displayName 
                      ...
                      <div className="ml-3 flex-1">
                        <div className="flex justify-between items-center">
                          <span className="font-medium">
                            {conversation.otherUser ? conversation.otherUser.displayName : 'Anonymous'}
                          </span>
        ...
        <div className="mt-10">
          <h2 className="text-xl font-bold text-purple-800 mb-4">My Confessions</h2>
...
function ChatWindow({ conversationId, currentUserId }: { conversationId: string; currentUserId: string }) {
  ...
  const renderTypingIndicator = () => {
    ...
    const typingUserNames = typingUsers.map(user => user.displayName).join(', ');
  ...
  const handleToggleBlock = async () => {
    ...
    // Confirm action
    if (!confirm(`Are you sure you want to ${action} ${otherUser.displayName || 'this user'}?${isBlocked ? '' : ' You won\'t be able to send or receive messages from them.'}`)) {
      return;
    }
    ...
    try {
      ...
      alert(`${otherUser.displayName || 'User'} has been ${action}ed successfully.`);
      ...
      <div className="p-4 border-b border-gray-200 flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          ...
          <button
            ...
            className={`${isBlocked ? 'text-green-500 hover:text-green-700' : 'text-red-500 hover:text-red-700'} focus:outline-none`}
      ...
      {isBlocked ? (
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <p className="text-gray-700 font-medium">You have blocked this user.</p>
            <p className="text-gray-500 text-sm mt-1 mb-4">You cannot send or receive messages from this user.</p>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <p className="text-gray-700 font-medium">You have been blocked by this user.</p>
            <p className="text-gray-500 text-sm mt-1 mb-4">You cannot send or receive messages from this user until they unblock you.</p>
          </div>
        </div>
      ) : (
        <>
                    ...
                    >
                      {!message.isCurrentUser && (
                        <div className="w-8 h-8 bg-purple-200 rounded-full flex items-center justify-center text-purple-700 font-semibold mr-2 self-end">
                          {message.sender?.displayName?.substring(0, 1)?.toUpperCase() || 'A'}
                        </div>
                      )}
                      <div 
                        className={`max-w-[75%] rounded-lg p-3 ${
                          message.isCurrentUser ? 'bg-purple-100' : 'bg-gray-100'
                        }`}
                      >
