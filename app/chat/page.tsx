...
export default function ChatPage() {
  ...
  // Track if data is already fetched to avoid redundant fetches
  ...
  useEffect(() => {
    ...
    } else if (status === "authenticated" && !hasDataBeenFetched.current) {
      setIsLoading(false);
      fetchConversations();
...
function MyConfessions({ userId }: { userId: string }) {
  ...
  // Cache control to prevent unnecessary API calls
  const dataFetchedRef = useRef(false);
  const lastFetchTime = useRef(0);
  const CACHE_DURATION = 60000; // Cache data for 1 minute
  
  const fetchConfessions = useCallback(async (force = false) => {
    // Don't fetch if we already have data and it's not stale, unless forced
    const now = Date.now();
    if (!force && confessions.length > 0 && now - lastFetchTime.current < CACHE_DURATION) {
      setIsLoading(false);
      return;
    }
  ...
  const handleDeleteConfession = async (confessionId: string) => {
    ...
    try {
      ...
      // Update the UI immediately without making another API call
